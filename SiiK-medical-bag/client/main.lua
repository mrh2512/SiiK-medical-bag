...
local function loadModel(model)
    -- Accepts either a model name (string) or a model hash (number/backticks).
    local hash = (type(model) == 'number') and model or joaat(model)

    if not IsModelInCdimage(hash) then
        -- Fallback to a very common vanilla prop so the script never hard-fails due to a missing DLC prop.
        local fallback = joaat('prop_ld_health_pack')
        if IsModelInCdimage(fallback) then
            dbg('Model not in CD image, falling back to prop_ld_health_pack:', model)
            hash = fallback
        else
            return false
        end
...
RegisterNetEvent('SiiK-medical-bag:client:BeginPlace', function(itemData)
    ...
    ghostEnt = CreateObject(hash, pcoords.x, pcoords.y, pcoords.z, false, false, false)
